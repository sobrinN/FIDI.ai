...
export const Navbar: React.FC<NavbarProps> = ({ scrollY, onAccessSystem, onLoginClick, onAgentsClick, currentUser, onLogout }) => {
        ...
        <div className="hidden md:flex items-center space-x-12">
          {navItems.map((item) => (
            <a 
              ...
              href={item.href}
              onClick={(e) => {
                 if (item.label === 'Agentes' && onAgentsClick) {
                   e.preventDefault();
                   onAgentsClick();
                 }
              }}
             ...
             <div className="flex flex-col items-center gap-8 w-full px-8">
                {navItems.map((item) => (
                    ...
                    onClick={(e) => {
                        if (item.label === 'Agentes' && onAgentsClick) {
                            e.preventDefault();
                            handleMobileClick(onAgentsClick);
                        } else {
                            handleMobileClick();
                        }
                    }}
